...
class ObstacleAvoidingBot(Node):
    ...
    def get_scan_values(self,scan_data):
        ...
        self.regions = {
        'right':   int(min(min(scan_data.ranges[120:160])  , 100)),
        'mid':     int(min(min(scan_data.ranges[160:200]), 100)),
        'left':    int(min(min(scan_data.ranges[200:240]), 100)),
        'back':    int(min(min(scan_data.ranges[360:720])   , 100))
        }
        print(self.regions['left']," / ",self.regions['mid']," / ",self.regions['right'],self.regions['back'])
